...
- `Cache/`, `Verse/`, `Newsletter/` unchanged · `Bindings/` Sources (incl. `ttm/author-name` formats `by|name|byline-link|url`; only `ttm/meta-line` and `byline-link` may return HTML), Values (`footer_line( site, year, author )`, `author_line()`)
- `Blocks/` Registrar, Helpers · `Editor/` · `Templates/` Hierarchy · `Nav/` CurrentSection · `Rest/` · `Admin/` · `Compat/` Theme · `Plugin.php`
