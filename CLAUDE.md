...
- `themes/ttm-theme/assets/css/ttm.css` ≤ `cssBudgetBytes` (63488 ⚠️ ASSUMPTION, `scripts/check-budget.mjs`, raised from 61440 in R1-06 to restore SPEC/PLAN-named declarations dropped to fit, then from 62464 in R4-01 for the series-row count/Writing body layout fixes) — plain CSS, presets via `var(--wp--preset--…)`, no hex literals; every `--wp--preset--font-size--*`/`--color--*` reference is generated by a slug (rule 44, `scripts/check-theme-json.mjs`).
...
- `.is-style-rule-1`/`.is-style-rule-2` are full width (rule 35); no `core/group` with `is-style-grid-*` uses `constrained`/`flow`, and no `layout: constrained` group sits inside one (rule 36 extended). SPEC §3.1: grid column children are `layout: default`/`flow`, never `flex` or `constrained`; `ttm.css` owns their internal layout (e.g. `display: flex; flex-direction: column`) via a `ttm-*` selector, not the block's own `layout` attribute. Grep `is-style-grid` in `themes/ttm-theme/**`.
