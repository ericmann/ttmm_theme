...
/**
 * `npm run screenshots` (SPEC §6.10, P0-08, formerly P0-01/P0-12): writes each named set of
 * comparison screenshots from the running, seeded site. Requires `wp-env start` + `wp ttm seed
 * --reset` first. `SETS.owner` is this flight's six files into `docs/feedback/phase-5/` (see
 * that directory's own `README.md`); `docs/feedback/phase-4/` is history and is left as it was.
 * No live set this flight (SPEC §6.10: "Live screenshots are not taken this flight") -- a
 * `readme` set (`.github/screenshots/`, `--readme` on the command line) lands in P3-01.
 */
...
/**
 * Every named capture run: `outDir` to write into, `zones` to capture. `P3-01` adds a `readme`
 * set here for `--readme`.
 *
