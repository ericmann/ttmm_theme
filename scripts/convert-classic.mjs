 ...
 *                      "report":{"blockCounts":{...},"freeform":0,"html":"...","textEqual":true}}
 *
...
export function convertPost( post, editor ) {
	...
	for ( const block of blockList ) {
		...
		if ( block.name === 'core/freeform' || block.name === 'core/html' ) {
			freeform += 1;
		}
	}
	...
	return {
		...
		blocks: serialized,
		footnotes,
		report: { blockCounts, freeform, html: serialized, textEqual },
	};
}
...
function main() {
	...
	for ( const result of results ) {
		...
		if ( ! allowFreeform && result.report.freeform > 0 ) {
			console.error(
				`post ${ result.id } (${ result.slug }): ${ result.report.freeform } freeform/html block(s)`
			);
			failed = true;
		}
	}
