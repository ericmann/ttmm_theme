...
| `ttm/author-name` | `format: by\|name\|byline-link\|url` | “by Eric Mann” / “Eric Mann” / a linked byline (rich text, `core/paragraph` `content` only) / the author URL |

`ttm/author-name` reads `Config::author_name()`/`Config::author_url()`, backed by two new `Config`
keys: `site.author_name` (`'Eric Mann'`) and `site.author_url` (`'https://eric.mann.blog'`) —
the only place the owner's literal name lives in the plugin (every reader calls the accessor).
Empty name → `''` (the bound paragraph renders nothing).
...
## 10. WP-CLI
`wp ttm seed [--reset] [--starter-only] [--state=quiet|empty] [--no-demo-images] [--now="<Y-m-d H:i:s>"]` (demo/starter content, refuses in production; `--no-demo-images` falls back to the rule-45 placeholder even when a fixture names a real demo photograph; `--now` pins `Clock::now()` for the run, so a demo build is reproducible from any wall-clock day) · `wp ttm stats:flush` (clears the cached front-page/category stats transients) · `wp ttm verse fetch|inspect|log` · `wp ttm recount [--all]` (word counts) · `wp ttm primary:assign [--dry-run] [--from-yoast]` (Yoast's own primary category first, else nav-order fallback) · `wp ttm series:assign <series-slug> --from-tags=<tag,tag,…> [--from-tag=<tag>] [--form=] [--status=] [--total=] [--name=] [--dry-run]` (create/update a series from tags, number by date) · `wp ttm series:rebuild` · `wp ttm convert:export --all-classic|--post=<id> --out=<file>` → `scripts/convert-classic.mjs` → `wp ttm convert:import <file> [--dry-run] [--allow-freeform]` · `wp ttm convert:revert --post=<id>|--all` · `wp ttm audit [--format=csv] [--only=<flag,…>] [--summary]` · `wp ttm migrate:politics [--dry-run]` · `wp ttm migrate:redirects [--format=nginx]` · `wp ttm migrate:close-comments [--dry-run]` · `wp ttm migrate:syndication [--dry-run] [--post=<id>]` · `wp ttm migrate:excerpts --from=yoast [--dry-run]` · `wp ttm migrate:images [--hosts=<host,host,…>] [--dry-run]` (`--hosts` optional, defaults to `migration.image_hosts`, SPEC §5) · `wp ttm demo:options` (prints the demo build's `setSiteOptions` payload as one JSON line: `blogname`, `blogdescription`, `timezone_string`, `permalink_structure`, `ttm_books` with every `cover_id` zeroed, `ttm_verse`, `ttm_verse_history`, `ttm_settings` forced to `{"newsletter":{"provider":"none","endpoint":""}}`) · `wp ttm demo:verify [--posts=<n>] [--pages=<n>] [--series=<n>] [--attachments=<n>]` (the demo blueprint's own post-import assertion: post/page/series/attachment counts, every series term has both `ttm_status` and `ttm_form`, no published post left on the default/`uncategorized` primary category).
