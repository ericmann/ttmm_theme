...
## 10. WP-CLI
`wp ttm verse fetch|inspect|log` · `wp ttm recount [--all]` (word counts) · `wp ttm primary:assign [--dry-run]` · `wp ttm series:assign <series-slug> --from-tag=<tag>` (create series from a tag, number by date) · `wp ttm migrate:politics [--to=child|tag]`.
