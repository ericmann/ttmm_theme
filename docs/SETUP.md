...
The `fidelity` Playwright project (also driven by `npm run test:e2e`, `--config tests/e2e/playwright.config.mjs`) runs `tests/e2e/fidelity.spec.mjs` and `tests/e2e/editors.spec.mjs` — one `test()` per `docs/SPEC.md §6.2` table row, transcribed verbatim, each checking a single computed-style/text/count assertion against the seeded front page at the mock's viewport (`tests/e2e/lib/presets.mjs` reads colours/sizes from `theme.json` rather than hard-coding hex; `tests/e2e/lib/style.mjs` wraps `getComputedStyle()`). Every row starts as `test.fixme` (SPEC §3.2 rule 38: a front-page CSS change without its row passing is rejected); each later flight task un-fixmes the rows it makes pass, and the flight isn't done until none remain. `desktop`/`phone` (the phase 1 projects) ignore these two files; `fidelity` ignores everything else.
