...
| Ingress | Cloudflare Tunnel (`cloudflared` Deployment, token in a Secret) → Service | No public IP, no cert-manager needed. Staging hostname behind Cloudflare Access. Restore the real client IP from `CF-Connecting-IP` (see note below) at this hop -- WordPress otherwise sees every visitor as the tunnel/ingress's own address. |
